...
const TIMELINE = [
  { title: 'First Meeting',      text: 'The day our paths crossed for the very first time, without either of us knowing what it would become.' },
  { title: 'First Conversation', text: 'Hours felt like minutes. We talked about everything and nothing, and I didn\'t want it to end.' },
  { title: 'First Date',         text: 'Nervous hands, shy smiles, and a night I still remember in perfect detail.' },
  { title: 'First Selfie',       text: 'Proof that even our silliest, unplanned moments turned into memories worth keeping.' },
  { title: 'First Trip',         text: 'New places, same us — that\'s when I knew home wasn\'t a place, it was you.' },
  { title: 'First Anniversary',  text: 'Three hundred and sixty five days later, and I\'d choose you in every single one of them again.' },
];
...
TIMELINE.forEach((item, i) => {
  ...
  div.innerHTML = `
    ...
    <div class="tl-card">
      <div class="tl-photo"><div class="ph-photo"><span>💞</span><small>Photo ${i + 1}</small></div></div>
      <h3>${item.title}</h3>
      <p>${item.text}</p>
    </div>`;
...
for (let i = 1; i <= GALLERY_COUNT; i++) {
  const ar = [1, 0.8, 1.3, 1][i % 4];
  galleryPhotos.push({ n: i, ar });
  const div = document.createElement('div');
  div.className = 'gallery-item';
  div.style.setProperty('--ar', ar);
  div.innerHTML = `<div class="ph-photo"><span>❤</span><small>Photo ${i}</small></div>`;
...
function renderLightbox() {
  ...
  lightboxInner.innerHTML = `<div class="ph-photo" style="height:100%;"><span style="font-size:2.4rem;">❤</span><small>Photo ${p.n}</small></div>`;
}
